...
chao1_data <- read.table(tsv_path, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
head(chao1_data)

# Rename the first column to "SampleID_full" and the second column to "simpson"
names(chao1_data)[1] <- "SampleID_full"
names(chao1_data)[2] <- "chao1"
# Expected output:
#   SampleID_full   chao1
#   KA001           0.8831962
#   KA011           0.9256630
...
my_data <- merge(my_data, chao1_data, by = "SampleID_full", all.x = TRUE)
head(my_data)


# Filter out any rows missing Chao1 data
my_data <- my_data[!is.na(my_data$chao1), ]
...
plot_data <- plot_data[, c("chao1", "MicroplasticLength", "MicroplasticConcentrationNumeric", "Parasite")]
# Create the plot
library(ggplot2)

p <- ggplot(plot_data, aes(x = MicroplasticConcentrationNumeric, y = chao1, 
